...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        batch_ids = Batch.swe_fundamentals_objects.filter(start_date=batch_start_date).values_list('pk', flat=True)
        ...
        for section_id in section_ids:
            ...
            slack_user_ids = list(StudentUser.objects.filter(pk__in=student_user_ids).filter(slack_user_id__isnull=False).values_list('slack_user_id', flat=True))
