...
from staff.views.coupon import ListView
...
def test_coupon_list_anonymous_user_redirected_to_login():
    request = RequestFactory().get('/coupons/')
    request.user = AnonymousUser()

    response = ListView.as_view()(request)

    assert response.status_code == HttpResponseRedirect.status_code
    assert 'staff/login/?next=/coupons/' in response.url

def test_coupon_list_template_rendered_for_logged_in_user(logged_in_existing_user):
