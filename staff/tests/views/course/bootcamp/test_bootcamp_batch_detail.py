...
from staff.views.course.bootcamp.bootcamp_batch import DetailView
...
def test_template_rendered_if_batch_exists(coding_bootcamp_batch, existing_user):
    ...
    assert response.status_code == HttpResponse.status_code
    assert 'course/bootcamp/batch/detail.html' in (template.name for template in response.templates)
